...
export default function ArticleHeader({ transparent = false, brandName = 'The Insider', logoLetter = 'T', logoColor = '#0F4C81' }: ArticleHeaderProps) {
    return (
        <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${transparent ? 'bg-transparent border-transparent' : 'bg-white border-b border-gray-200 shadow-sm'}`}>
            <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
                <Link href="/" className="flex items-center gap-2 group">
                    <div
                        className={`w-8 h-8 flex items-center justify-center font-serif font-bold text-xl rounded-sm transition-colors ${transparent ? 'bg-white text-gray-900' : 'text-white'}`}
                        style={transparent ? undefined : { backgroundColor: logoColor }}
                    >
                        {logoLetter}
                    </div>
